...
const getAllBooksHandler = (request, h) => {
  ...
  if (name) {
    ...
    queryBooks = queryBooks.filter((book) => Number(book.reading) === Number(reading));
  } if (finished) {
    queryBooks = queryBooks.filter((book) => Number(book.finished) === Number(finished));
  }

  const response = h.response({
    status: 'success',
    data: {
      books: queryBooks.map((book) => ({ id: book.id, name: book.name, publisher: book.publisher })),
    },
  });
